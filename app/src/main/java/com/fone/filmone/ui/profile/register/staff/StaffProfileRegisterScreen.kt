...
fun StaffProfileRegisterScreen(
    ...
    LaunchedEffect(key1 = uiState.staffProfileRegisterUiEvent) {
        listenUiEvent(
            uiEvent = uiState.staffProfileRegisterUiEvent,
...
private fun listenUiEvent(
    uiEvent: StaffProfileRegisterUiEvent,
    navController: NavHostController
) {
    when (uiEvent) {
        StaffProfileRegisterUiEvent.Clear -> Unit
        StaffProfileRegisterUiEvent.RegisterComplete -> {
            navController.popBackStack()
        }
    }
}
